function MakeTask(id, title, desc, teg, complete){
    ...
    if(complete == "true") // "Выполнено (checkbox)"
    {
...
function firstStart(){
    ...
    if(window.tasks == null){ // При первой загрузке
       window.lastId = 0;
...
function DeleteTask(el){ // Удаление задачи;
...
function CompleteTask(el){ // Выполнено - в конец списка;
    ...
    div.classList.toggle('complete');
    if(div.classList.length == 0){ // Добавление в начало;
...
function Delete(n){ //Удаление первого или последнего элемента;
    let allTasks = document.getElementById("allTasks");
...
function DeleteClass(){ //Удаление всех классов выделения;
    let all = document.getElementsByClassName("task");
    if(all.length > 0){
